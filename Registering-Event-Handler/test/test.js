...
describe('', function() {
  it('', function() {
    ...
    let handlerCallbacks = {
      "$event, $handler": function(event, handler){
        ...
        if(handler.name !== 'showInfo'){
          return {failure: "Did you pass in `showInfo` as the event handler function for the second argument of the `.addEventListener()` method?"}
        }
        return true;
      }
    }
    ...
    expect(isMatchThree, handlerCallbacks.failure || 'Did you correctly assign an event handler function as the second argument of `.addEventListener()` function?').to.not.be.false;
  });
});
